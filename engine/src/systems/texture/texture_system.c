...
void texture_system_release(const char* name) {
    ...
    texture_reference ref;
    if (state_ptr && hashtable_get(&state_ptr->registered_texture_table, name, &ref)) {
        // Prevent invalid release when reference_count is already zero.
        if (ref.reference_count <= 0) {
            // KWARN("Release called on texture '%s' with invalid ref_count=%d", name, ref.reference_count);
            return;
        }
        ...
        // If reference count is zero and auto_release is enabled, destroy texture.
        if (ref.reference_count == 0 && ref.auto_release) {
            ...
            // Reset texture memory and identifiers.
            kzero_memory(t, sizeof(texture));
            t->id = INVALID_ID;
            t->generation = INVALID_ID;

            // Reset reference entry.
            ref.handle = INVALID_ID;
            ref.auto_release = false;

            KTRACE("Released texture '%s'. Texture unloaded (ref_count=0, auto_release=true).", name);
        } else {
            KTRACE("Released texture '%s', ref_count=%d (auto_release=%s).",
                   name, ref.reference_count, ref.auto_release ? "true" : "false");
        }

        // Update hashtable entry with modified reference.
        hashtable_set(&state_ptr->registered_texture_table, name, &ref);
    } else {
        // Texture not found in table.
        KERROR("texture_system_release failed: texture '%s' not found.", name);
    }
}
